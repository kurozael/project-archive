...
local sliceMat = "severance/gradient/severance_secondary_gradient";
...
function THEME:CreateFonts()
	...
	{
		font		= "Gobold Bold",
		size		= Clockwork.kernel:FontScreenScale(0),
		weight		= 600,
		antialiase	= true,
		additive 	= false
	});
...
function THEME:Initialize()
	local infoColor = Color(100, 125, 150, 255);
	local infoColorDarkish = Color(75, 100, 125, 255);
	local infoColorDark = Color(50, 75, 100, 255);
	...
	Clockwork.option:SetColor("basic_form_highlight", colorBlack);
	Clockwork.option:SetColor("basic_form_color", colorBlack);

	Clockwork.option:SetColor("columnsheet_shadow_normal", colorBlack);
	Clockwork.option:SetColor("columnsheet_text_normal", colorBlack);
	Clockwork.option:SetColor("columnsheet_shadow_active", colorBlack);
	Clockwork.option:SetColor("columnsheet_text_active", colorBlack);
	
	Clockwork.option:SetColor("scoreboard_name", infoColorDarkish);
	Clockwork.option:SetColor("scoreboard_desc", infoColor);
	Clockwork.option:SetColor("scoreboard_item_background", Color(0, 0, 0, 175));
	...
	--[[ Misc Keys --]]
	--Clockwork.option:SetKey("intro_image", "severance/severance_logo"); we don't need this
	Clockwork.option:SetKey("schema_logo", "severance/severance_logo");
	...
	Clockwork.option:SetKey("menu_music", "ambient/atmosphere/tone_alley.wav");
	Clockwork.option:SetKey("gradient", "severance/gradient/severance_default_gradient");
	...
	SLICED_PROGRESS_BAR = Clockwork.render:AddSlice9("SimpleSev", sliceMat, 28);
	...
	SLICED_COLUMNSHEET_BUTTON = Clockwork.render:AddSlice9("SimpleSevButton", "severance/sliced/severance_button_slice", 20);
end;

local DIRTY_TEXTURE = Material("severance/gradient/severance_secondary_gradient.png");
local SCRATCH_TEXTURE = Material("severance/gradient/severance_alternative_gradient.png");
local BAR_TEXTURE = Material("severance/gradient/bars.png");
