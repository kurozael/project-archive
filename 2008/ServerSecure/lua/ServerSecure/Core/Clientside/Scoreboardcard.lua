...
function PANEL:Init()
	...
	self.HTML = vgui.Create("SS HTML", self)
end
...
function PANEL:UpdatePlayerData()
	...
	if (SS) then
		for K, V in pairs(SS.Part.List) do
			if (V[2] == "Hover") then
				local Explode = string.Explode(": ", self.Player:GetNetworkedString(V[1]))
				
				if Explode[2] then
					self:SetInfo(1, Explode[1]..":", Explode[2])
				end
			end
		end
	end
...
function PANEL:PerformLayout()	
	...
	self.HTML:SetPos(self:GetWide() - 70, 40)
	self.HTML:SetSize(64, 64)
end
