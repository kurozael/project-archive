...
function ENTITY:__init(arguments)
	if (self.m_bUseSprite) then
		...
		self:SetSize( self.m_sprite:GetW(), self.m_sprite:GetH() );
	end;
...
-- Called when the entity's data has changed.
function ENTITY:OnDataChanged(key, value) end;
...
-- Called when the entity's data keys are requested.
function ENTITY:OnRequestDataKeys() end;
...
function ENTITY:InitPhysicsBox(width, height)
	if (self.m_physBody) then
		...
		self.m_physBody = g_Physics:CreateBox(width, height);
...
function ENTITY:InitPhysicsCircle(radius)
	if (self.m_physBody) then
		...
		self.m_physBody = g_Physics:CreateCircle(radius);
		self.m_physBody:SetData( tostring(self.m_iEntIndex) );
		self:SetCollisionType(COLLISION_STATIC);
	end;
...
function ENTITY:SetHealth(health)
	self.m_iHealth = math.min(math.max(health, 0), 100);
end;
...
function ENTITY:SetMaterial(material, bNoPhysicsUpdate)
...
function ENTITY:EmitSound(fileName)
	...
	if (player) then
		...
		g_Sounds:PlaySound(fileName, volume);
	else
		g_Sounds:PlaySound(fileName, 1);
	end;
end;

-- A function to set entity data.
function ENTITY:SetData(key, value)
	self:OnDataChanged(key, value);
	self.m_data[key] = value;
end;

-- A function to get entity data.
...
function ENTITY:SetCollisionType(collisionType)
	...
	self.m_physBody:SetSensor(collisionType == COLLISION_NONE);
...
function ENTITY:DrawBBox()
	local position = self:GetPos();
	local height = self:GetPhysH();
	local width = self:GetPhysW();
	
	g_Render:DrawBox( position.x, position.y, width, height, Color(1, 0, 0, 1) );
end;
