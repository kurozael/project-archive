			...
			if (IsValid(trace.Entity)) then
				...
				if (trace.Entity:IsPlayer() or trace.Entity:IsNPC()) then
					...
					timer.Simple(FrameTime() * 0.5, function()
						if (IsValid(trace.Entity)) then
							trace.Entity:TakeDamageInfo(Clockwork:FakeDamageInfo(self.Primary.Damage, self, self, trace.HitPos, DMG_CLUB, 2));
						end;
					end);
				else
					...
					if (IsValid(physicsObject)) then
						...
						if (!player) then
							timer.Simple(FrameTime() * 0.5, function()
								if (IsValid(trace.Entity)) then
									trace.Entity:TakeDamageInfo(Clockwork:FakeDamageInfo((self.Primary.Damage * 2), self, self.Owner, trace.HitPos, DMG_CLUB, 2));
								end;
							end);
						else
							timer.Simple(FrameTime() * 0.5, function()
								if (IsValid(trace.Entity)) then
									trace.Entity:TakeDamageInfo(Clockwork:FakeDamageInfo(self.Primary.Damage, self, self.Owner, trace.HitPos, DMG_CLUB, 2));
								end;
							end);
						end;
					end;
