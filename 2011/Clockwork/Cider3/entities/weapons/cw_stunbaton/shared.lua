			...
			if (IsValid(trace.Entity)) then
				...
				if (trace.Entity:IsPlayer()) then
					...
					if (trace.Entity:Health() > 25) then
						trace.Entity:TakeDamageInfo(Clockwork:FakeDamageInfo(5 + strength, self, self.Owner, trace.HitPos, DMG_CLUB, 2));
					end;
				...
				elseif (IsValid(trace.Entity:GetPhysicsObject())) then
					...
					if (!player or player:Health() > 25) then
						if (!player) then
							trace.Entity:TakeDamageInfo(Clockwork:FakeDamageInfo(10 + (strength * 2), self, self.Owner, trace.HitPos, DMG_CLUB, 2));
						else
							trace.Entity:TakeDamageInfo(Clockwork:FakeDamageInfo(5 + strength, self, self.Owner, trace.HitPos, DMG_CLUB, 2));
						end;
					end;
