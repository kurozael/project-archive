...
-- Called each frame.
function ENT:Think()
	local playerEyePos = CloudScript.Client:EyePos();
	...
	if ( IsValid(player) ) then
		...
		if ( (eyePos:Distance(playerEyePos) > 32 or GetViewEntity() != CloudScript.Client
		or CloudScript.Client != player or !isPlayer) and ( !isPlayer or player:Alive() ) ) then
