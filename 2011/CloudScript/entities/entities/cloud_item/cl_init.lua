...
function ENT:HUDPaintTargetID(x, y, alpha)
	...
	if (index != 0) then
		...
		if (itemTable) then
			...
			y = CloudScript:DrawInfo(itemTable.name, x, y, color, alpha);
			
			if (itemTable.weightText) then
				y = CloudScript:DrawInfo(itemTable.weightText, x, y, colorWhite, alpha);
			else
				y = CloudScript:DrawInfo(itemTable.weight.."kg", x, y, colorWhite, alpha);
			end;
		end;
	end;
end;
...
function ENT:Think()
	...
	if (itemTable) then
		...
		CloudScript.plugin:Call("CloudScriptItemEntityThink", itemTable, self);
	end;
end;
...
function ENT:Draw()
	...
	if (index != 0) then
		...
		if (itemTable) then
			...
			if (CloudScript.plugin:Call("CloudScriptItemEntityDraw", itemTable, self) == false) then
				drawModel = false;
			end;
		end;
	end;
