...
function SWEP:PrimaryAttack()
	...
	if (SERVER) then
		local action = CloudScript.player:GetAction(self.Owner);
		local trace = self.Owner:GetEyeTraceNoCursor();
		
		if (self.Owner:GetPos():Distance(trace.HitPos) <= 192) then
			if ( IsValid(trace.Entity) ) then
				local info = CloudScript.plugin:Call("PlayerGetLockInfo", self.Owner, trace.Entity);
				
				if ( info and CloudScript.plugin:Call("PlayerCanLockEntity", self.Owner, trace.Entity) ) then
					...
					if (isNotLocking or isNotUnlocking) then
						...
						CloudScript.player:SetAction(player, "lock", info.duration);
						
						CloudScript.player:EntityConditionTimer(player, target, nil, info.duration, 192, function()
							if ( !CloudScript.plugin:Call("PlayerCanLockEntity", player, target) ) then
								return false;
							else
							...
							if (success) then
								...
								CloudScript.player:SetAction(player, "lock", false);
							end;
						end);
					end;
				end;
...
function SWEP:SecondaryAttack()
	...
	if (SERVER) then
		...
		if (self.Owner:GetPos():Distance(trace.HitPos) <= 192) then
			if ( IsValid(trace.Entity) ) then
				local info = CloudScript.plugin:Call("PlayerGetUnlockInfo", self.Owner, trace.Entity);
				
				if ( info and CloudScript.plugin:Call("PlayerCanUnlockEntity", self.Owner, trace.Entity) ) then
					local isNotUnlocking = (action != "unlock");
					local isNotLocking = (action != "lock");
					...
					if (isNotLocking or isNotUnlocking) then
						...
						CloudScript.player:SetAction(player, "unlock", info.duration);
						
						CloudScript.player:EntityConditionTimer(player, target, nil, info.duration, 192, function()
							if ( !CloudScript.plugin:Call("PlayerCanUnlockEntity", player, target) ) then
								return false;
							else
								return player:Alive() and !player:IsRagdolled() and player:IsUsingKeys();
							end;
							...
							if (success) then
								...
								CloudScript.player:SetAction(player, "unlock", false);
							end;
						end);
					end;
				end;
