...
function SWEP:PunchEntity()
	...
	if (SERVER) then
		...
		if ( IsValid(traceLineAttack.Entity) ) then
			traceLineAttack.Entity:TakeDamageInfo( CloudScript:FakeDamageInfo(self.Primary.Damage, self, self.Owner, traceLineAttack.HitPos, DMG_CLUB, 1) );
		end;
	end;
end;
...
function SWEP:PrimaryAttack()
	if (SERVER) then
		if ( CloudScript.plugin:Call("PlayerCanThrowPunch", self.Owner) ) then
			...
			if (self.Owner:GetShootPos():Distance(trace.HitPos) <= 64) then
				if ( IsValid(trace.Entity) ) then
					if (trace.Entity:IsPlayer() or trace.Entity:IsNPC() or trace.Entity:GetClass() == "prop_ragdoll") then
						...
						and CloudScript.plugin:Call("PlayerCanPunchKnockout", self.Owner, trace.Entity) ) then
							CloudScript.player:SetRagdollState(trace.Entity, RAGDOLL_KNOCKEDOUT, 15);
							
							CloudScript.plugin:Call("PlayerPunchKnockout", self.Owner, trace.Entity);
						elseif ( CloudScript.plugin:Call("PlayerCanPunchEntity", self.Owner, trace.Entity) ) then
							self:PunchEntity();
							
							CloudScript.plugin:Call("PlayerPunchEntity", self.Owner, trace.Entity);
						end;
					...
					elseif ( IsValid( trace.Entity:GetPhysicsObject() ) ) then
						if ( CloudScript.plugin:Call("PlayerCanPunchEntity", self.Owner, trace.Entity) ) then
							self:PunchEntity();
			...
			CloudScript.plugin:Call("PlayerPunchThrown", self.Owner);
			...
			CloudScript.plugin:Call("PlayerAdjustNextPunchInfo", self.Owner, info);
...
function SWEP:SecondaryAttack()
	if (SERVER) then
		...
		if ( IsValid(trace.Entity) and CloudScript.entity:IsDoor(trace.Entity) ) then
			if (self.Owner:GetShootPos():Distance(trace.HitPos) <= 64) then
				if ( CloudScript.plugin:Call("PlayerCanKnockOnDoor", self.Owner, trace.Entity) ) then
					self:PlayKnockSound();
					
					self.Weapon:SetNextPrimaryFire(CurTime() + 0.25);
					self.Weapon:SetNextSecondaryFire(CurTime() + 0.25);
					
					CloudScript.plugin:Call("PlayerKnockOnDoor", self.Owner, trace.Entity);
				end;
			end;
		end;
	end;
