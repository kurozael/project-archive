...
function PLUGIN:CloudScriptInitPostEntity()
	if ( CloudScript.config:Get("remove_map_physics"):Get() ) then
...
function PLUGIN:InitPostEntity()
	...
	if ( CloudScript.config:Get("remove_map_physics"):Get() ) then
		for k, v in ipairs( CloudScript:GetPhysicsEntities() ) do
			v:Remove();
		end;
	end;
	...
	if (string.lower( game.GetMap() ) == "rp_tb_city45_v02n") then
		for k, v in ipairs( ents.FindInSphere(Vector(226.2188, 4550, 238.0313), 32) ) do
			if ( CloudScript.entity:IsDoor(v) ) then
				v:Remove(); break;
			end;
		end;
	...
	if (string.lower( game.GetMap() ) == "rp_evocity_v2d") then
		...
		for k, v in ipairs(entities) do
			v:Spawn(); CloudScript.entity:MakeSafe(v, true, true, true);
		end;
	end;
end;