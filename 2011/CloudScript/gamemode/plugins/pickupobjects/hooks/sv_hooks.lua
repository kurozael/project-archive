...
function PLUGIN:PlayerGiveWeapons(player)
	if ( CloudScript.config:Get("take_physcannon"):Get() ) then
		CloudScript.player:TakeSpawnWeapon(player, "weapon_physcannon");
	end;
end;
...
function PLUGIN:CloudScriptConfigChanged(key, data, previousValue, newValue)
	if (key == "take_physcannon") then
		
		for k, v in ipairs( _player.GetAll() ) do
			if (newValue) then
				CloudScript.player:TakeSpawnWeapon(v, "weapon_physcannon");
			else
				CloudScript.player:GiveSpawnWeapon(v, "weapon_physcannon");
			end;
		end;
	end;
end;
		...
		if ( !IsValid(player.holdingEntity) ) then
			if (key == IN_ATTACK2) then
				...
				if (IsValid(entity) and trace.HitPos:Distance( player:GetShootPos() ) <= 192) then
					...
					if ( canPickup and !CloudScript.entity:IsDoor(entity) ) then
