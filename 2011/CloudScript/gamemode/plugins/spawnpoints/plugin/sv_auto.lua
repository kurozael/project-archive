...
function PLUGIN:LoadSpawnPoints()
	...
	local spawnPoints = CloudScript:RestoreSchemaData( "plugins/spawnpoints/"..game.GetMap() );
	
	for k, v in pairs(spawnPoints) do
		local faction = CloudScript.faction:Get(k);
		local class = CloudScript.class:Get(k);
		local name;
...
function PLUGIN:SaveSpawnPoints()
	...
	CloudScript:SaveSchemaData("plugins/spawnpoints/"..game.GetMap(), spawnPoints);
end;