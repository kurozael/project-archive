...
function ENT:TalkToPlayer(player, text, default)
	CloudScript.player:Notify(player, self:GetSharedVar("name").." says \""..(text or default).."\"");
end;
...
function ENT:MakeChatBubble()
	self.ChatBubble = ents.Create("cloud_chatbubble");
...
function ENT:Use(activator, caller)
	if ( IsValid(activator) and activator:IsPlayer() ) then
		if (activator:GetEyeTraceNoCursor().HitPos:Distance( self:GetPos() ) < 196) then
			if (CloudScript.plugin:Call("PlayerCanUseSalesman", activator, self) != false) then
				CloudScript.plugin:Call("PlayerUseSalesman", activator, self);
			end;
		end;
	end;
end;