...
function COMMAND:OnRun(player, arguments)
	...
	if (!areaPointData or areaPointData.name != name) then
		...
		if ( CloudScript:ToBool( arguments[3] ) ) then
			player.areaPointData.expires = true;
		end;
		
		CloudScript.player:Notify(player, "You have added the minimum point, now add the maximum point.");
	elseif (!areaPointData.maximum) then
		areaPointData.maximum = trace.HitPos;
		
		CloudScript.player:Notify(player, "You have added the minimum point, now add the text point.");
	elseif (!areaPointData.position) then
		...
		CloudScript:StartDataStream( nil, "AreaAdd", data );
		
		PLUGIN.areaDisplays[#PLUGIN.areaDisplays + 1] = data;
		PLUGIN:SaveAreaDisplays();
		
		CloudScript.player:Notify(player, "You have added the '"..areaPointData.name.."' area display.");
		
		player.areaPointData = nil;
	end;
end;

CloudScript.command:Register(COMMAND, "AreaAdd");
...
function COMMAND:OnRun(player, arguments)
	...
	for k, v in pairs(PLUGIN.areaDisplays) do
		if (string.lower(v.name) == name) then
			...
			or v.position:Distance(position) <= 256) then
				CloudScript:StartDataStream( nil, "AreaRemove", {
					name = v.name,
					minimum = v.minimum,
					maximum = v.maximum
				} );
	...
	if (removed > 0) then
		if (removed == 1) then
			CloudScript.player:Notify(player, "You have removed "..removed.." area display.");
		else
			CloudScript.player:Notify(player, "You have removed "..removed.." area displays.");
		end;
	else
		CloudScript.player:Notify(player, "There were no area displays near this position.");
	end;
	
	PLUGIN:SaveAreaDisplays();
end;

CloudScript.command:Register(COMMAND, "AreaRemove");