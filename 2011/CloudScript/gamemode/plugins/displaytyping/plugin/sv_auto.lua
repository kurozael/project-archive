...
concommand.Add("cloud_typing_start", function(player, command, arguments)
	if ( player:Alive() and !player:IsRagdolled(RAGDOLL_FALLENOVER) ) then
		if ( arguments and arguments[1] ) then
			hook.Call( "PlayerStartTypingDisplay", CloudScript, player, arguments[1] );
...
concommand.Add("cloud_typing_finish", function(player, command, arguments)
	if ( IsValid(player) ) then
		if (arguments and arguments[1] and arguments[1] == "1") then
			CloudScript.plugin:Call("PlayerFinishTypingDisplay", player, true);
		else
			CloudScript.plugin:Call("PlayerFinishTypingDisplay", player);
		end;
