...
function PLUGIN:UpdateAnimation(player, velocity, maxSeqGroundSpeed)
	if (CloudScript.Client == player) then
...
function PLUGIN:RenderScreenspaceEffects()
	cam.Start3D( EyePos(), EyeAngles() );
		if ( self:ShouldDrawLegs() ) then
			self.RenderPos = CloudScript.Client:GetPos();
			
			if ( CloudScript.Client:InVehicle() ) then
				self.RenderAngle = CloudScript.Client:GetVehicle():GetAngles();
				self.RenderAngle:RotateAroundAxis(self.RenderAngle:Up(), 90);
			else
				self.BiaisAngles = CloudScript.Client:EyeAngles();
				...
				if (CloudScript.Client:GetGroundEntity() == NULL) then
					self.RenderPos.z = self.RenderPos.z + 8;
					
					if ( CloudScript.Client:KeyDown(IN_DUCK) ) then
						self.RenderPos.z = self.RenderPos.z - 28;
					end;
				end;
			end;
			
			self.RenderColor.r, self.RenderColor.g, self.RenderColor.b, self.RenderColor.a = CloudScript.Client:GetColor();
