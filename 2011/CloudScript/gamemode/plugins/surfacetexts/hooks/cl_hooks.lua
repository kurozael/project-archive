	...
	CloudScript:OverrideMainFont(font);
		cam.Start3D(eyePos, eyeAngles);
			for k, v in pairs(self.surfaceTexts) do
				local alpha = math.Clamp(CloudScript:CalculateAlphaFromDistance(512, eyePos, v.position) * 1.5, 0, 255);
				...
				if (alpha > 0) then
					...
					cam.Start3D2D(v.position, v.angles, (v.scale or 0.25) * 0.2);
						for k2, v2 in ipairs(text) do
							y = CloudScript:DrawInfo(v2, 0, y, colorWhite, alpha, nil, function(x, y, width, height)
								return x, y - (height / 2);
							end, 3);
						end;
					cam.End3D2D();
