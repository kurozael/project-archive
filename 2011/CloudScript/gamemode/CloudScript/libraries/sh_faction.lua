...
CloudScript.faction = {};
CloudScript.faction.stored = {};
CloudScript.faction.buffer = {};
...
function CloudScript.faction:Register(data, name)
	if (data.models) then
...
function CloudScript.faction:GetLimit(name)
...
function CloudScript.faction:IsGenderValid(faction, gender)
...
function CloudScript.faction:IsModelValid(faction, gender, model)
...
function CloudScript.faction:Get(name)
	if (name) then
...
function CloudScript.faction:GetPlayers(faction)
	local players = {};
	...
	for k, v in ipairs( _player.GetAll() ) do
		if ( v:HasInitialized() ) then
			...
			elseif (CloudScript.player:GetFaction(v) == faction) then
				players[#players + 1] = v;
			end;
		end;
	end;
...
if (SERVER) then
	function CloudScript.faction:HasReachedMaximum(player, faction)
		local factionTable = self:Get(faction);
	...
	function CloudScript.faction:HasReachedMaximum(faction)
		local factionTable = self:Get(faction);
