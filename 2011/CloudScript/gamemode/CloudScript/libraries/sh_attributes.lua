...
CloudScript.attributes = {};

if (SERVER) then
	function CloudScript.attributes:Progress(player, attribute, amount, gradual)
		local attributeTable = CloudScript.attribute:Get(attribute);
		...
		if (attributeTable) then
			...
			amount = amount * CloudScript.config:Get("scale_attribute_progress"):Get();
			
			if ( attributes[attribute] ) then
				if (attributes[attribute].amount == attributeTable.maximum) then
					if (amount > 0) then
						return false, "You have the maximum of this "..CloudScript.option:GetKey("name_attribute", true).."!";
					end;
				end;
			else
	...
	function CloudScript.attributes:Update(player, attribute, amount)
		local attributeTable = CloudScript.attribute:Get(attribute);
		local attributes = player:QueryCharacter("attributes");
		...
		if (attributeTable) then
			...
			elseif (attributes[attribute].amount == attributeTable.maximum) then
				if (amount and amount > 0) then
					return false, "You have reached the maximum of this "..CloudScript.option:GetKey("name_attribute", true).."!";
				end;
			end;
			...
			umsg.Start("cloud_AttributesUpdate", player);
			...
			CloudScript.plugin:Call("PlayerAttributeUpdated", player, attributeTable, amount);
	...
	function CloudScript.attributes:ClearBoosts(player)
		umsg.Start("cloud_AttributesBoostClear", player);
		umsg.End();
	...
	function CloudScript.attributes:IsBoostActive(player, identifier, attribute, amount, duration)
	...
	function CloudScript.attributes:Boost(player, identifier, attribute, amount, duration)
		local attributeTable = CloudScript.attribute:Get(attribute);
		...
		if (attributeTable) then
			...
			if (amount) then
				...
				umsg.Start("cloud_AttributesBoost", player);
					umsg.Long(attributeTable.index);
	...
	function CloudScript.attributes:Fraction(player, attribute, fraction, negative)
		local attributeTable = CloudScript.attribute:Get(attribute);
	...
	function CloudScript.attributes:Get(player, attribute, boostless, negative)
		local attributeTable = CloudScript.attribute:Get(attribute);
		
		if (attributeTable) then
			attribute = attributeTable.uniqueID;
			
			if ( CloudScript:HasObjectAccess(player, attributeTable) ) then
				local maximum = attributeTable.maximum;
	...
	CloudScript.attributes.stored = {};
	CloudScript.attributes.boosts = {};
	
	-- A function to get the attributes panel.
	function CloudScript.attributes:GetPanel()
		return self.panel;
	end;
	
	-- A function to get the local player's attribute as a fraction.
	function CloudScript.attributes:Fraction(attribute, fraction, negative)
		local attributeTable = CloudScript.attribute:Get(attribute);
	...
	function CloudScript.attributes:Get(attribute, boostless, negative)
		local attributeTable = CloudScript.attribute:Get(attribute);
		
		if (attributeTable) then
			attribute = attributeTable.uniqueID;
			
			if ( CloudScript:HasObjectAccess(CloudScript.Client, attributeTable) ) then
				local maximum = attributeTable.maximum;
	...
	usermessage.Hook("cloud_AttributesBoostClear", function(msg)
		local index = msg:ReadLong();
		local identifier = msg:ReadString();
		local attributeTable = CloudScript.attribute:Get(index);
		
		if (attributeTable) then
			local attribute = attributeTable.uniqueID;
			
			if (identifier and identifier != "") then
				if ( CloudScript.attributes.boosts[attribute] ) then
					CloudScript.attributes.boosts[attribute][identifier] = nil;
				end;
			else
				CloudScript.attributes.boosts[attribute] = nil;
			end;
		else
			CloudScript.attributes.boosts = {};
		end;
		...
		if ( CloudScript.menu:GetOpen() ) then
			local panel = CloudScript.attributes:GetPanel();
	...
	usermessage.Hook("cloud_AttributesBoost", function(msg)
		local index = msg:ReadLong();
		...
		if (attributeTable) then
			...
			if ( !CloudScript.attributes.boosts[attribute] ) then
				CloudScript.attributes.boosts[attribute] = {};
			end;
			
			if (amount == 0) then
				CloudScript.attributes.boosts[attribute][identifier] = nil;
			elseif (duration > 0 and endTime > 0) then
				CloudScript.attributes.boosts[attribute][identifier] = {
					duration = duration,
					endTime = endTime,
					default = amount,
					amount = amount
	...
	usermessage.Hook("cloud_AttributeProgress", function(msg)
		local index = msg:ReadLong();
		...
		if (attributeTable) then
			...
			if ( CloudScript.attributes.stored[attribute] ) then
				CloudScript.attributes.stored[attribute].progress = amount;
			else
				CloudScript.attributes.stored[attribute] = {amount = 0, progress = amount};
			end;
		end;
	end);
	
	usermessage.Hook("cloud_AttributesUpdate", function(msg)
		local index = msg:ReadLong();
		local amount = msg:ReadLong();
		local attributeTable = CloudScript.attribute:Get(index);
		...
		if (attributeTable) then
			...
			if ( CloudScript.attributes.stored[attribute] ) then
				CloudScript.attributes.stored[attribute].amount = amount;
			else
				CloudScript.attributes.stored[attribute] = {amount = amount, progress = 0};
			end;
		end;
	end);
	
	usermessage.Hook("cloud_AttributesClear", function(msg)
		CloudScript.attributes.stored = {};
		CloudScript.attributes.boosts = {};
