...
function CloudScript.entity:IsDoor(entity)
...
function CloudScript.entity:IsDecaying(entity)
	return entity.decaying;
end;

-- A function to check if an entity is in a box.
function CloudScript.entity:IsInBox(entity, minimum, maximum)
...
function CloudScript.entity:GetPelvisPosition(entity)
	local position = entity:GetPos();
...
function CloudScript.entity:CanSeePosition(entity, position, allowance, ignoreEnts)
	local trace = {};
...
function CloudScript.entity:CanSeeNPC(entity, target, allowance, ignoreEnts)
	local trace = {};
...
function CloudScript.entity:CanSeePlayer(entity, target, allowance, ignoreEnts)
...
function CloudScript.entity:CanSeeEntity(entity, target, allowance, ignoreEnts)
	local trace = {};
...
function CloudScript.entity:GetPlayer(entity)
	local player = entity:GetNetworkedEntity("player");
...
function CloudScript.entity:IsInteractable(entity)
	local class = entity:GetClass();
...
function CloudScript.entity:IsPhysicsEntity(entity)
...
function CloudScript.entity:IsPodEntity(entity)
...
function CloudScript.entity:IsChairEntity(entity)
...
if (SERVER) then
	function CloudScript.entity:OpenDoor(entity, delay, unlock, sound, origin)
		if ( self:IsDoor(entity) ) then
			if (unlock) then
	...
	function CloudScript.entity:MakeSafe(entity, physgunProtect, toolProtect, freezeEntity)
	...
	function CloudScript.entity:StatueRagdoll(entity, forceLimit)
	...
	function CloudScript.entity:DropItemsAndCash(inventory, cash, position, entity)
	...
	function CloudScript.entity:MakeIntoRagdoll(entity, force, overrideVelocity, overrideAngles)
	...
	function CloudScript.entity:IsDoorUnsellable(door)
		return door.unsellable;
	end;
	
	-- A function to set a door's parent.
	function CloudScript.entity:SetDoorParent(door, parent)
	...
	function CloudScript.entity:IsDoorParent(door)
		return table.Count( self:GetDoorChildren(door) ) > 0;
	end;

	-- A function to get a door's parent.
	function CloudScript.entity:GetDoorParent(door)
		if ( IsValid(door.doorParent) ) then
	...
	function CloudScript.entity:GetDoorChildren(door)
		return door.doorChildren or {};
	end;
	
	-- A function to set a door as unownable.
	function CloudScript.entity:SetDoorUnownable(entity, unownable)
		if ( self:IsDoor(entity) ) then
			if (unownable) then
				entity:SetNetworkedBool("unownable", true);
				
				if ( self:GetOwner(entity) ) then
					CloudScript.player:TakeDoor(self:GetOwner(entity), entity, true);
	...
	function CloudScript.entity:SetDoorFalse(entity, isFalse)
		if ( self:IsDoor(entity) ) then
			if (isFalse) then
	...
	function CloudScript.entity:SetDoorHidden(entity, hidden)
		if ( self:IsDoor(entity) ) then
			if (hidden) then
				self:SetDoorUnownable(entity, true);
	...
	function CloudScript.entity:SetDoorSharedAccess(entity, sharedAccess)
	...
	-- A function to set whether a door has shared access.
	function CloudScript.entity:SetDoorSharedText(entity, sharedText)
		if ( self:IsDoorParent(entity) ) then
	...
	function CloudScript.entity:DoorHasSharedAccess(entity)
		return entity.sharedAccess;
	end;
	
	-- A function to get whether a door has shared text.
	function CloudScript.entity:DoorHasSharedText(entity)
		return entity.sharedText;
	end;
	
	-- A function to set a door's text.
	function CloudScript.entity:SetDoorText(entity, text)
		if ( self:IsDoor(entity) ) then
			if ( self:IsDoorParent(entity) ) then
	...
	function CloudScript.entity:SetDoorName(entity, name)
		if ( self:IsDoor(entity) ) then
			if (name) then
	...
	function CloudScript.entity:SetChairAnimations(entity)
	...
	function CloudScript.entity:SetStartAngles(entity, angles)
		entity.startAngles = angles;
	end;
	
	-- A function to get an entity's start angles.
	function CloudScript.entity:GetStartAngles(entity)
		return entity.startAngles;
	end;
	
	-- A function to set an entity's start position.
	function CloudScript.entity:SetStartPosition(entity, position)
		entity.startPosition = position;
	end;
	
	-- A function to get an entity's start position.
	function CloudScript.entity:GetStartPosition(entity)
		return entity.startPosition;
	end;
	
	-- A function to return an entity's collision group.
	function CloudScript.entity:ReturnCollisionGroup(entity, collisionGroup)
		if ( IsValid(entity) ) then
			...
			if ( IsValid(physicsObject) ) then
				if ( !physicsObject:IsPenetrating() ) then
					...
					CloudScript:CreateTimer("collision_group_"..index, 1, 1, function()
	...
	function CloudScript.entity:SetMapEntity(entity, isMapEntity)
		if (isMapEntity) then
			CloudScript.Entities[entity] = entity;
		else
			CloudScript.Entities[entity] = nil;
		end;
	end;
	
	-- A function to get whether an entity is a map entity.
	function CloudScript.entity:IsMapEntity(entity)
		if ( CloudScript.Entities[entity] ) then
			return true;
		end;
	end;
	
	-- A function to make an entity flush with the ground.
	function CloudScript.entity:MakeFlushToGround(entity, position, normal)
		entity:SetPos( position + ( entity:GetPos() - entity:NearestPoint( position - (normal * 512) ) ) );
	end;
	
	-- A function to make an entity disintegrate.
	function CloudScript.entity:Disintegrate(entity, delay, velocity, Callback)
		if (velocity) then
	...
	function CloudScript.entity:SetPlayer(entity, player)
		entity:SetNetworkedEntity("player", player);
	end;
	
	-- A function to make an entity decay.
	function CloudScript.entity:Decay(entity, seconds, Callback)
		...
		CloudScript:CreateTimer("decay_"..index, 1, 0, function()
			...
			if ( IsValid(entity) ) then
				...
				if (a <= 0) then
					...
					CloudScript:DestroyTimer("decay_"..index);
				else
	...
	function CloudScript.entity:CreateCash(player, cash, position, angles)
		if ( CloudScript.config:Get("cash_enabled"):Get() ) then
			local entity = ents.Create("cloud_cash");
			
			if (type(player) == "table") then
				if (player.key and player.uniqueID) then
					CloudScript.player:GivePropertyOffline(player.key, player.uniqueID, entity, true);
				end;
			elseif ( IsValid(player) ) then
				CloudScript.player:GiveProperty(player, entity);
			end;
	...
	function CloudScript.entity:CreateGenerator(player, class, position, angles)
		...
		elseif ( IsValid(player) ) then
			CloudScript.player:GiveProperty(player, entity, true);
		end;
	...
	function CloudScript.entity:CreateShipment(player, item, batch, position, angles)
		local entity = ents.Create("cloud_shipment");
	...
	function CloudScript.entity:CreateItem(player, class, position, angles)
		local itemTable = CloudScript.item:Get(class);
		local entity = ents.Create("cloud_item");
	...
	function CloudScript.entity:CopyOwner(entity, target)
		...
		CloudScript.player:GivePropertyOffline(key, uniqueID, target, networked, removeDelay);
	end;
	
	-- A function to get whether an entity belongs to a player's other character.
	function CloudScript.entity:BelongsToAnotherCharacter(player, entity)
		local uniqueID = self:QueryProperty(entity, "uniqueID");
		local key = self:QueryProperty(entity, "key");
	...
	function CloudScript.entity:SetPropertyVar(entity, key, value)
		if (entity.property) then entity.property[key] = value; end;
	end;
	
	-- A function to query an entity's property table.
	function CloudScript.entity:QueryProperty(entity, key, default)
	...
	function CloudScript.entity:ClearProperty(entity)
		local owner = self:GetOwner(entity);

		if (owner) then
			CloudScript.player:TakeProperty(owner, entity);
		elseif ( self:HasOwner(entity) ) then
			local uniqueID = self:QueryProperty(entity, "uniqueID");
			local key = self:QueryProperty(entity, "key");
			
			CloudScript.player:TakePropertyOffline(key, uniqueID, entity);
		end;
	end;

	-- A function to get whether an entity has an owner.
	function CloudScript.entity:HasOwner(entity)
		return self:QueryProperty(entity, "owned");
	end;
	
	-- A function to get an entity's owner.
	function CloudScript.entity:GetOwner(entity)
	...
	function CloudScript.entity:CalculateDoorTextPosition(door, reversed)
		local traceData = {};
	...
	function CloudScript.entity:ForceMenuOption(entity, option, arguments)
		CloudScript:StartDataStream( "EntityMenuOption", {entity, option, arguments} );
	end;
