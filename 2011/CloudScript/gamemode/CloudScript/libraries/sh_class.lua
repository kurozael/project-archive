...
CloudScript.class = {};
CloudScript.class.stored = {};
CloudScript.class.buffer = {};

-- A function to register a new class.
function CloudScript.class:Register(data, name)
	...
	data.index = CloudScript:GetShortCRC(name);
	data.name = name;
...
function CloudScript.class:GetLimit(name)
...
function CloudScript.class:Get(name)
	if (name) then
...
function CloudScript.class:AssignToDefault(player)
	...
	if (#defaultClasses > 0) then
		...
		for k, v in pairs(self.stored) do
			if ( CloudScript:HasObjectAccess(player, v) ) then
				return self:Set(player, v.index);
			end;
		end;
	end;
...
function CloudScript.class:GetAppropriateModel(name, player, noSubstitute)
	local defaultModel;
	local class = self:Get(name);
	local model;
	local skin;
	...
	if (class) then
		model, skin = self:GetModelByGender( name, CloudScript.player:GetGender(player) );
...
function CloudScript.class:GetModelByGender(name, gender)
...
function CloudScript.class:HasAnyFlags(name, flags)
...
function CloudScript.class:HasFlags(name, flags)
	local query = self:Query(name, "flags")
...
function CloudScript.class:Query(name, key, default)
	local class = self:Get(name);
...
if (SERVER) then
	function CloudScript.class:Set(player, name, noRespawn, addDelay, noModelChange)
		local weapons = CloudScript.player:GetWeapons(player);
		local oldClass = self:Get( player:Team() );
		local newClass = self:Get(name);
		local ammo = CloudScript.player:GetAmmo(player, !player.firstSpawn);
		
		if (newClass) then
			player:SetTeam(newClass.index);
			
			if (!noModelChange) then
				CloudScript:PlayerSetModel(player);
			end;
			...
			if (!noRespawn) then
				...
				elseif ( !player:IsRagdolled() ) then
					CloudScript.player:LightSpawn(player, weapons, ammo);
				end;
			end;
			
			if (addDelay) then
				player.nextChangeClass = CurTime() + CloudScript.config:Get("change_class_interval"):Get();
			end;
			
			CloudScript.plugin:Call("PlayerClassSet", player, newClass, oldClass, noRespawn, addDelay, noModelChange);
