...
function ITEM:HasPlayerEquipped(player, arguments)
	if (SERVER or !arguments) then
		...
		for k, v in pairs(weapons) do
			if (CloudScript.item:GetWeapon(v) == self) then
				return true;
			end;
		end;
...
function ITEM:OnPlayerUnequipped(player, arguments)
	...
	if (IsValid(weapon) and CloudScript.item:GetWeapon(weapon) == self) then
		local class = weapon:GetClass();
		
		if (arguments != "drop") then
			if ( CloudScript.plugin:Call("PlayerCanHolsterWeapon", player, self) ) then
				if ( player:UpdateInventory(self.uniqueID, 1) ) then
					player:StripWeapon(class);
					player:SelectWeapon("cloud_hands");
					
					CloudScript.plugin:Call("PlayerHolsterWeapon", player, self);
				end;
			end;
		elseif ( CloudScript.plugin:Call("PlayerCanDropWeapon", player, self) ) then
			local trace = player:GetEyeTraceNoCursor();
			
			if (player:GetShootPos():Distance(trace.HitPos) <= 192) then
				local entity = CloudScript.entity:CreateItem(player, self.uniqueID, trace.HitPos);
				...
				if ( IsValid(entity) ) then
					...
					CloudScript.plugin:Call("PlayerDropWeapon", player, self, entity);
				end;
			else
...
function ITEM:HandleAmmo(player, itemEntity)
	if (itemEntity.data.sClip) then
		local sClip = CloudScript.player:GetSecondaryAmmo(player, self.weaponClass, true);
		
		if (sClip == 0) then
			CloudScript.player:SetSecondaryAmmo(player, self.weaponClass, itemEntity.data.sClip);
		else
	...
	if (itemEntity.data.pClip) then
		local pClip = CloudScript.player:GetPrimaryAmmo(player, self.weaponClass, true);
		
		if (pClip == 0) then
			CloudScript.player:SetPrimaryAmmo(player, self.weaponClass, itemEntity.data.pClip);
		else
