...
function PANEL:Populate()
	...
	self.questionsForm:SetName( CloudScript.quiz:GetName() );
	...
	for k, v in pairs(questions) do
		if (ScrW() < 1024) then
			...
			function panel:OnSelect(index, value, data)
				CloudScript:StartDataStream( "QuizAnswer", {key, index} );
			end;
	...
	self.continueButton:SetCallback(function(panel)
		CloudScript:StartDataStream("QuizCompleted", true);
	end);
...
vgui.Register("cloud_Quiz", PANEL, "DPanel");