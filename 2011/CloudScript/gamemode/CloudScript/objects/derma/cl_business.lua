...
function PANEL:Init()
	...
	self:SetTitle( CloudScript.option:GetKey("name_business") );
...
function PANEL:Rebuild()
	...
	for k, v in pairs( CloudScript.item:GetAll() ) do
		if (!v.isBaseItem and v.business and v.cost and v.batch) then
			if ( CloudScript:HasObjectAccess(CloudScript.Client, v) ) then
				if ( CloudScript.plugin:Call("PlayerCanSeeBusinessItem", v.uniqueID) ) then
	...
	CloudScript.plugin:Call("PlayerBusinessRebuilt", self, categories);
	...
	if (table.Count(categories) > 0) then
		for k, v in pairs(categories) do
			...
			table.sort(v.items, function(a, b)
				if (CloudScript.item.stored[a].cost == CloudScript.item.stored[b].cost) then
					return CloudScript.item.stored[a].name < CloudScript.item.stored[b].name;
				else
					return CloudScript.item.stored[a].cost > CloudScript.item.stored[b].cost;
				end;
			end);
			
			for k2, v2 in pairs(v.items) do
				self.currentItem = v2;
				panelList:AddItem( vgui.Create("cloud_BusinessItem", self) );
			end;
		...
		local label = vgui.Create("cloud_InfoText", self);
			label:SetText("You do not have access to the "..CloudScript.option:GetKey("name_business", true).." menu!");
...
vgui.Register("cloud_Business", PANEL, "DFrame");
...
vgui.Register("cloud_BusinessCustom", PANEL, "DPanel");
...
function PANEL:Init()
	...
	if (CloudScript.OrderCooldown and curTime < CloudScript.OrderCooldown) then
		self.spawnIcon:SetCooldown(CloudScript.OrderCooldown);
	end;
	...
	function self.spawnIcon.DoClick(spawnIcon)
		...
		CloudScript:RunCommand("OrderShipment", self.itemTable.uniqueID);
	end;
...
function PANEL:Think()
	...
	local customText = CloudScript.plugin:Call("GetCustomBusinessInfoLabel", self, self.itemTable);
...
vgui.Register("cloud_BusinessItem", PANEL, "DPanel");