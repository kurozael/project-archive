...
function PANEL:Init()
	...
	self:SetTitle( CloudScript.option:GetKey("name_inventory") );
	...
	CloudScript.inventory.panel = self;
	CloudScript.inventory.panel:Rebuild();
end;
...
function PANEL:Rebuild()
	...
	self.weightForm:AddItem( vgui.Create("cloud_InventoryWeight", self) );
	...
	for k, v in pairs(CloudScript.inventory.stored) do
	...
	local itemWeapons = {};
	local fakeWeapons = CloudScript.Client:GetWeapons();
	...
	for k, v in pairs(CloudScript.item.stored) do
		...
		if ( v.HasPlayerEquipped and v:HasPlayerEquipped(CloudScript.Client, itemWeapons) ) then
	...
	CloudScript.plugin:Call("PlayerInventoryRebuilt", self, inventoryCategories);
	...
	if (#equippedCategories > 0) then
		...
		for k, v in ipairs(equippedCategories) do
			...
			for k2, v2 in pairs(v) do
				local model, skin = CloudScript.item:GetIconInfo(v2);
				...
				self.customData.Callback = function()
					if (v2.OnHandleUnequip) then
						v2:OnHandleUnequip(function(arguments)
							if (arguments) then
								CloudScript:StartDataStream( "UnequipItem", {v2.uniqueID, arguments} );
							else
								CloudScript:StartDataStream("UnequipItem", v2.uniqueID);
							end;
						end);
					else
						CloudScript:StartDataStream("UnequipItem", v2.uniqueID);
				...
				panelList:AddItem( vgui.Create("cloud_InventoryCustom", self) ) ;
			end;
		end;
	...
	if (#inventoryCategories > 0) then
		...
		self.inventoryForm:SetName( CloudScript.option:GetKey("name_inventory") );
		...
		for k, v in ipairs(inventoryCategories) do
			...
			for k2, v2 in pairs(v) do
				...
				panelList:AddItem( vgui.Create("cloud_InventoryItem", self) ) ;
			end;
		end;
...
vgui.Register("cloud_Inventory", PANEL, "DFrame");
...
function PANEL:Init()
	...
	if (customData.description) then
		toolTip = CloudScript.config:Parse(customData.description);
	end;
...
vgui.Register("cloud_InventoryCustom", PANEL, "DPanel");

local PANEL = {};

-- Called when the panel is initialized.
function PANEL:Init()
	local destroyName = CloudScript.option:GetKey("name_destroy");
	local dropName = CloudScript.option:GetKey("name_drop");
	local useName = CloudScript.option:GetKey("name_use");
	
	self:SetSize(self:GetParent():GetWide(), 32);
	
	self.itemTable = table.Copy( CloudScript.item:Get(self:GetParent().currentItem) );
	self.amount = CloudScript.inventory.stored[self.itemTable.uniqueID];
	...
	self.spawnIcon.OpenMenu = function(spawnIcon)
		if (self.itemTable.OnHandleRightClick) then
			...
			if (customFunction and customFunction != "Use") then
				...
				RunConsoleCommand( "cloud", "InvAction", self.itemTable.uniqueID, string.lower(customFunction) );
		...
		if (self.itemTable.OnUse) then
			if (self.itemTable.OnHandleUse) then
				self.itemTable:OnHandleUse(function()
					CloudScript:RunCommand("InvAction", self.itemTable.uniqueID, "use");
				end);
			else
				CloudScript:RunCommand("InvAction", self.itemTable.uniqueID, "use");
			end;
	...
	function self.spawnIcon.DoClick(spawnIcon)
		...
		CloudScript.plugin:Call("PlayerAdjustItemFunctions", self.itemTable, itemFunctions);
		
		CloudScript:ValidateTableKeys(itemFunctions);
		...
		if (#itemFunctions > 0) then
			...
			CloudScript.plugin:Call("PlayerAdjustItemMenu", self.itemTable, menu, itemFunctions);
			...
			for k, v in pairs(itemFunctions) do
				...
				elseif ( (!self.itemTable.dropText and v == "Drop") or (self.itemTable.dropText and v == self.itemTable.dropText) ) then
					menu:AddOption(v, function()
						if (self.itemTable) then
							CloudScript:RunCommand("InvAction", self.itemTable.uniqueID, "drop");
						end;
					end);
				elseif ( (!self.itemTable.destroyText and v == "Destroy") or (self.itemTable.destroyText and v == self.itemTable.destroyText) ) then
					...
					subMenu:AddOption("Yes", function()
						if (self.itemTable) then
							CloudScript:RunCommand("InvAction", self.itemTable.uniqueID, "destroy");
						end;
					end);
					...
					menu:AddOption(v, function()
						if (self.itemTable) then
							RunConsoleCommand("cloud", "InvAction", self.itemTable.uniqueID, v);
						end;
					end);
				end;
			end;
...
vgui.Register("cloud_InventoryItem", PANEL, "DPanel");

local PANEL = {};

-- Called when the panel is initialized.
function PANEL:Init()
	local maximumWeight = CloudScript.inventory:GetMaximumWeight();
	...
	function self.spaceUsed.Paint(spaceUsed)
		local maximumWeight = CloudScript.inventory:GetMaximumWeight();
		local inventoryWeight = CloudScript.inventory:GetWeight();
...
function PANEL:Think()
	...
	self.weight:SetText(CloudScript.inventory:GetWeight().."/"..CloudScript.inventory:GetMaximumWeight().."kg");
	self.weight:SetPos(self:GetWide() / 2 - self.weight:GetWide() / 2, self:GetTall() / 2 - self.weight:GetTall() / 2);
	self.weight:SizeToContents();
end;
	
vgui.Register("cloud_InventoryWeight", PANEL, "DPanel");