...
function PANEL:Init()
	if ( !CloudScript.theme:Call("PreCharacterMenuInit", self) ) then
		...
		local hugeTextFont = CloudScript.option:GetFont("menu_text_huge");
		local scrH = ScrH();
		local scrW = ScrW();
		...
		self.titleLabel = vgui.Create("cloud_LabelButton", self);
		self.titleLabel:SetDisabled(true);
		self.titleLabel:SetFont(hugeTextFont);
		self.titleLabel:SetText( string.upper( CloudScript.schema:GetName() ) );
		...
		self.subLabel = vgui.Create("cloud_LabelButton", self);
		self.subLabel:SetDisabled(true);
		self.subLabel:SetFont(smallTextFont);
		self.subLabel:SetText( string.upper( CloudScript.schema:GetDescription() ) );
		...
		self.authorLabel = vgui.Create("cloud_LabelButton", self);
		self.authorLabel:SetDisabled(true);
		self.authorLabel:SetFont(tinyTextFont);
		self.authorLabel:SetText( "DEVELOPED BY "..string.upper( CloudScript.schema:GetAuthor() ) );
		...
		self.createButton = vgui.Create("cloud_LabelButton", self);
		...
		self.createButton:SetCallback(function(panel)
			if ( table.Count( CloudScript.character:GetAll() ) >= CloudScript.player:GetMaximumCharacters() ) then
				CloudScript.character:SetFault("You cannot create any more characters!");
			else
				self:OpenPanel("cloud_CharacterStageOne");
			end;
		end);
		...
		self.loadButton = vgui.Create("cloud_LabelButton", self);
		...
		self.loadButton:SetCallback(function(panel)
			self:OpenPanel("cloud_CharacterList", nil, function(panel)
				CloudScript.character:RefreshPanelList();
			end);
		end);
		...
		self.disconnectButton = vgui.Create("cloud_LabelButton", self);
		...
		self.disconnectButton:SetCallback(function(panel)
			if ( CloudScript.Client:HasInitialized() and !CloudScript.character:IsMenuReset() ) then
				CloudScript.character:SetPanelOpen(false);
			else
...
function PANEL:ReturnToMainMenu()
	...
	if (panel) then
		panel:FadeOut(0.5, function()
			CloudScript.character.activePanel = nil;
			panel:Remove();
		end);
	end;
...
function PANEL:OpenPanel(vguiName, childData, Callback)
	if ( !CloudScript.theme:Call("PreCharacterMenuOpenPanel", self, vguiName, childData, Callback) ) then
		local panel = CloudScript.character:GetActivePanel();
		...
		if (panel) then
			panel:FadeOut(0.5, function()
				...
				CloudScript.character.activePanel = vgui.Create(vguiName, self);
				CloudScript.character.activePanel:SetAlpha(0);
				CloudScript.character.activePanel:FadeIn(0.5);
				CloudScript.character.activePanel:MakePopup();
				CloudScript.character.activePanel:SetPos( ScrW() * 0.2, ScrH() * 0.2 );
				
				if (Callback) then
					Callback(CloudScript.character.activePanel);
				end;
			end);
		else
			...
			CloudScript.character.activePanel:FadeIn(0.5);
			CloudScript.character.activePanel:MakePopup();
			CloudScript.character.activePanel:SetPos( ScrW() * 0.2, ScrH() * 0.25 );
		...
		CloudScript.theme:Call("PostCharacterMenuOpenPanel", self);
...
function PANEL:Paint()
	if ( !CloudScript.theme:Call("PreCharacterMenuPaint", self) ) then
		local schemaLogo = CloudScript.option:GetKey("schema_logo");
		...
		CloudScript.theme:Call("PostCharacterMenuPaint", self)
	end;
...
function PANEL:Think()
	if ( !CloudScript.theme:Call("PreCharacterMenuThink", self) ) then
		local characters = table.Count( CloudScript.character:GetAll() );
		local loading = CloudScript.character:IsPanelLoading();
		local fault = CloudScript.character:GetFault();
		
		if ( CloudScript.plugin:Call("ShouldDrawCharacterBackgroundBlur") ) then
			CloudScript:RegisterBackgroundBlur(self, self.createTime, 0.5);
		else
		...
		if ( characters >= CloudScript.player:GetMaximumCharacters()
		or CloudScript.character:IsPanelLoading() ) then
		...
		CloudScript.theme:Call("PostCharacterMenuThink", self)
	end;
end;

vgui.Register("cloud_CharacterMenu", PANEL, "DPanel");

hook.Add("VGUIMousePressed", "CloudScript.character:VGUIMousePressed", function(panel, code)
	local characterPanel = CloudScript.character:GetPanel();
	local activePanel = CloudScript.character:GetActivePanel();
	
	if (CloudScript.character:IsPanelOpen() and activePanel and characterPanel == panel) then
...
function PANEL:PerformLayout()
	...
	self:SetSize( math.min(CloudScript.character:GetListWidth(), ScrW() - 32), math.min(self.panelList.pnlCanvas:GetTall() + 8, ScrH() * 0.6) );
end;

vgui.Register("cloud_CharacterList", PANEL, "DFrame");
...
function PANEL:Init()
	...
	CloudScript.plugin:Call("GetCustomCharacterButtons", self.characterTable, buttons);
	...
	for k, v in pairs(buttons) do
		...
		function button.DoClick(button)
			local function Callback()
				CloudScript:StartDataStream( "InteractCharacter", {characterID = self.characterID, action = k} );
			end;
	...
	function self.useButton.DoClick(spawnIcon)
		CloudScript:StartDataStream( "InteractCharacter", {characterID = self.characterID, action = "use"} );
	end;
	...
	function self.deleteButton.DoClick(spawnIcon)
		CloudScript:AddMenuFromData( nil, {
			["Yes"] = function()
				CloudScript:StartDataStream( "InteractCharacter", {characterID = self.characterID, action = "delete"} );
			end,
			["No"] = function() end
		} );
	end;
	...
	function self.spawnIcon.DoClick(spawnIcon)
		local options = {};
		local panel = CloudScript.character:GetPanel();
		...
		CloudScript.plugin:Call("GetCustomCharacterOptions", self.characterTable, options, menu);
		
		CloudScript:AddMenuFromData(nil, options, function(menu, key, value)
			menu:AddOption(key, function()
				CloudScript:StartDataStream( "InteractCharacter", {characterID = self.characterID, action = value} );
			end);
		end);
	end;
end;
...
function PANEL:Think()
	...
	self.spawnIcon:SetToolTip( CloudScript.plugin:Call("GetCharacterPanelToolTip", self, self.faction, self.characterTable) );
	...
	CloudScript.character:SetListWidth(self.nameLabel:GetWide() + 76);
	CloudScript.character:SetListWidth( (#self.buttonPanels * 20) + 76 );
	CloudScript.character:SetListWidth(self.detailsLabel:GetWide() + 76);
end;
	
vgui.Register("cloud_CharacterPanel", PANEL, "DPanel");
...
function PANEL:SetModel(model)
	...
	if ( IsValid(self.modelPanel.Entity) ) then
		...
		local menuSequence = CloudScript.animation:GetMenuSequence(model, true);
...
vgui.Register("cloud_CharacterModel", PANEL, "DPanel");
...
function PANEL:Init()
	...
	if (parent) then
		...
		if (self.info) then
			factionTable = CloudScript.faction:Get(self.info.faction);
		end;
	end;
	...
	self.attributesForm:SetName( CloudScript.option:GetKey("name_attributes") );
	...
	self.continueButton = vgui.Create("cloud_LabelButton", self);
	...
	function self.continueButton.DoClick(button)
		...
		CloudScript:StartDataStream("CreateCharacter", self.info);
	end;
	
	for k, v in pairs( CloudScript.attribute:GetAll() ) do
...
vgui.Register("cloud_CharacterStageThree", PANEL, "DFrame");
...
function PANEL:Init()
	...
	local physDesc = (CloudScript.command:Get("CharPhysDesc") != nil);
	local parent = self:GetParent();
	local panel = CloudScript.character:GetPanel();
	...
	if (!CloudScript.faction.stored[self.faction].GetModel) then
		selectModel = true;
	end;
	
	local models = CloudScript.faction.stored[self.faction].models[ string.lower(parent.childData.gender) ];
	...
	if (!CloudScript.faction.stored[self.faction].GetName) then
		self.nameForm = vgui.Create("DForm");
		self.nameForm:SetPadding(4);
		self.nameForm:SetName("Name");
		
		if (CloudScript.faction.stored[self.faction].useFullName) then
	...
	function self.continueButton.DoClick(button)
		...
		local attributeTable = CloudScript.attribute:GetAll();
		...
		if (!CloudScript.faction.stored[self.faction].GetName) then
			if ( IsValid(self.fullNameTextEntry) ) then
				...
				if (info.fullName == "") then
					return CloudScript.character:SetFault("You did not choose a name, or the name that you chose is not valid!");
				end;
			else
				...
				if ( string.find(info.forename, "[%p%s%d]") or string.find(info.surname, "[%p%s%d]") ) then
					return CloudScript.character:SetFault("Your forename and surname must not contain punctuation, spaces or digits!");
				end;
				
				if ( !string.find(info.forename, "[aeiou]") or !string.find(info.surname, "[aeiou]") ) then
					return CloudScript.character:SetFault("Your forename and surname must both contain at least one vowel!");
				end;
				
				if ( string.len(info.forename) < 2 or string.len(info.surname) < 2) then
					return CloudScript.character:SetFault( "Your forename and surname must both be at least 2 characters long!");
				end;
				
				if ( string.len(info.forename) > 16 or string.len(info.surname) > 16) then
					return CloudScript.character:SetFault("Your forename and surname must not be greater than 16 characters long!");
				end;
			end;
		end;
		
		if (selectModel and !info.model) then
			return CloudScript.character:SetFault("You did not choose a model, or the model that you chose is not valid!");
		end;
		...
		if (physDesc) then
			...
			if (string.len(info.physDesc) < minimumPhysDesc) then
				return CloudScript.character:SetFault("The physical description must be at least "..minimumPhysDesc.." characters long!");
			end;
		end;
		
		CloudScript.plugin:Call("PlayerAdjustCharacterCreationInfo", self, info);
		
		if (attributes) then
			parent:OpenPanel("cloud_CharacterStageThree", info);
			
			return;
		end;
		
		CloudScript:StartDataStream("CreateCharacter", info);
	end;
	...
	CloudScript.plugin:Call("CharacterCreationPanelInitialized", self);
		...
		if (v.name == faction) then
			...
			if ( self.modelItemsList and v.models[lowerGender] ) then
				for k2, v2 in pairs( v.models[lowerGender] ) do
					spawnIcon = CloudScript:CreateColoredSpawnIcon(self);
...
vgui.Register("cloud_CharacterStageTwo", PANEL, "DFrame");
...
function PANEL:Init()
	...
	for k, v in pairs(CloudScript.faction.stored) do
		if ( !v.whitelist or CloudScript.character:IsWhitelisted(v.name) ) then
			if ( !CloudScript.faction:HasReachedMaximum(k) ) then
		...
		self.factionMultiChoice.OnSelect = function(multiChoice, index, value, data)
			for k, v in pairs(CloudScript.faction.stored) do
				if (v.name == value) then
					...
					CloudScript.CurrentFactionSelected = {self, value};
	...
	elseif (#factions == 1) then
		for k, v in pairs(CloudScript.faction.stored) do
			if ( v.name == factions[1] ) then
				...
				CloudScript.CurrentFactionSelected = {self, v.name};
	...
	function self.continueButton.DoClick(button)
		if ( IsValid(self.genderMultiChoice) ) then
			...
			for k, v in pairs(CloudScript.faction.stored) do
				if (v.name == faction) then
					if ( CloudScript.faction:IsGenderValid(faction, gender) ) then
						parent:OpenPanel( "cloud_CharacterStageTwo", {faction = faction, gender = gender} );
					end;
...
vgui.Register("cloud_CharacterStageOne", PANEL, "DFrame");

usermessage.Hook("cloud_CharacterRemove", function(msg)
	local characters = CloudScript.character:GetAll();
	...
	if (table.Count(characters) > 0) then
		...
		if ( characters[characterID] ) then
			...
			if ( !CloudScript.character:IsPanelLoading() ) then
				CloudScript.character:RefreshPanelList();
			end;
			
			if ( CloudScript.character:GetPanelList() ) then
				if (table.Count(characters) == 0) then
					CloudScript.character:GetPanel():ReturnToMainMenu();
				end;
			end;
		end;
	end;
...
CloudScript:HookDataStream("SetWhitelisted", function(data)
...
CloudScript:HookDataStream("CharacterAdd", function(data)
	CloudScript.character:GetAll()[data.characterID] = data;
...
CloudScript:HookDataStream("CharacterMenu", function(data)
	if (data == CHARACTER_MENU_LOADED) then
		if ( CloudScript.character:GetPanel() ) then
			CloudScript.character:SetPanelLoading(false);
			CloudScript.character:RefreshPanelList();
		end;
...
usermessage.Hook("cloud_CharacterOpen", function(msg)
	CloudScript.character:SetPanelOpen(true);
	
	if ( msg:ReadBool() ) then
		CloudScript.character.isMenuReset = true;
	end;
end);

usermessage.Hook("cloud_CharacterFinish", function(msg)
	if ( msg:ReadBool() ) then
		CloudScript.character:SetPanelMainMenu();
		CloudScript.character:SetPanelOpen(false, true);
		CloudScript.character:SetFault(nil);
	else
		CloudScript.character:SetFault( msg:ReadString() );
	end;
end);