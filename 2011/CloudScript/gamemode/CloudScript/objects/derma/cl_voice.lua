...
function PANEL:Setup(player)
	if ( !CloudScript.player:DoesRecognise(player, RECOGNISE_TOTAL) ) then
		local unrecognisedName, usedPhysDesc = CloudScript.player:GetUnrecognisedName(player);
	...
	if (self.Recognises) then
		...
		self.Avatar:SetImage("CloudScript/unknown");
	end;
