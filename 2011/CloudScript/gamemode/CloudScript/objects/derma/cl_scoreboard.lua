...
function PANEL:Init()
	...
	CloudScript.scoreboard = self;
	CloudScript.scoreboard:Rebuild();
end;
	...
	for k, v in ipairs( _player.GetAll() ) do
		if ( v:HasInitialized() ) then
			...
			if (class) then
				...
				if ( CloudScript.plugin:Call("PlayerShouldShowOnScoreboard", v) ) then
	...
	if (table.Count(classes) > 0) then
		...
		for k, v in pairs(classes) do
			...
			for k2, v2 in pairs(v.players) do
				...
				self.currentFaction = CloudScript.player:GetFaction(v2);
				...
				panelList:AddItem( vgui.Create("cloud_ScoreboardItem", self) ) ;
			end;
...
vgui.Register("cloud_Scoreboard", PANEL, "DFrame");
...
function PANEL:Init()
	SCOREBOARD_PANEL = true;
		...
		local info = {
			doesRecognise = CloudScript.player:DoesRecognise(parent.currentPlayer),
		...
		info.text = CloudScript.plugin:Call("GetPlayerScoreboardText", info.player);
		
		CloudScript.plugin:Call("ScoreboardAdjustPlayerInfo", info);
		...
		if (info.doesRecognise) then
			...
			self.spawnIcon:SetImage("CloudScript/unknown");
		...
		function self.spawnIcon.DoClick(spawnIcon)
			local options = {};
				CloudScript.plugin:Call("GetPlayerScoreboardOptions", info.player, options);
...
vgui.Register("cloud_ScoreboardItem", PANEL, "DPanel");