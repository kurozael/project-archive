...
function PANEL:Init()
	...
	self:SetTitle( CloudScript.option:GetKey("name_attributes") );
	...
	CloudScript.attributes.panel = self;
	CloudScript.attributes.panel.boosts = {};
	CloudScript.attributes.panel.progress = {};
	CloudScript.attributes.panel.attributes = {};
...
function PANEL:Rebuild()
	...
	for k, v in pairs( CloudScript.attribute:GetAll() ) do
		if ( CloudScript:HasObjectAccess(CloudScript.Client, v) ) then
			if (v.category) then
	...
	if (#categories > 0 or #miscellaneous > 0) then
		local attributeName = string.lower( CloudScript.option:GetKey("name_attribute") );
		...
		local attributesForm = vgui.Create("DForm", self);
			...
			attributesForm:SetName( CloudScript.option:GetKey("name_attributes") );
		self.panelList:AddItem(attributesForm);
		
		for k, v in ipairs(miscellaneous) do
			self.currentAttribute = v[1];
			attributesForm:AddItem( vgui.Create("cloud_AttributesItem", self) ) ;
		end;
		...
		for k, v in ipairs(categories) do
			...
			for k2, v2 in pairs(v.attributes) do
				self.currentAttribute = v2[1];
					categoryForm:AddItem( vgui.Create("cloud_AttributesItem", self) ) ;
			end;
		...
		local label = vgui.Create("cloud_InfoText", self);
			label:SetText("You do not have access to any "..CloudScript.option:GetKey("name_attributes", true).."!");
			label:SetInfoColor("red");
		self.panelList:AddItem(label);
	end;
...
vgui.Register("cloud_Attributes", PANEL, "DFrame");

local PANEL = {};

-- Called when the panel is initialized.
function PANEL:Init()
	self.attribute = CloudScript.attribute:Get(self:GetParent().currentAttribute);
	...
	self.percentageText:SetTextColor( CloudScript.option:GetColor("white") );
	...
	self.nameLabel:SetTextColor( CloudScript.option:GetColor("white") );
	...
	function self.baseBar.Paint(baseBar)
		...
		local attributes = CloudScript.attributes.panel.attributes;
		local mainColor = Color(139, 174, 179, 255);
		local frameTime = FrameTime() * 10;
		local uniqueID = self.attribute.uniqueID;
		local curTime = CurTime();
		local default = CloudScript.attributes.stored[uniqueID];
		local boosts = CloudScript.attributes.panel.boosts;
		local boost = 0;
		...
		if ( CloudScript.attributes.boosts[uniqueID] ) then
			for k, v in pairs( CloudScript.attributes.boosts[uniqueID] ) do
		...
		if (boostData.negative) then
			if (attributes[uniqueID] - boostData.boost < 0) then
				CloudScript:DrawSimpleGradientBox(0, 0, 0, boostData.width, baseBar:GetTall(), hinderColor);
			else
				boostData.width = math.min(boostData.width, width);
				
				CloudScript:DrawSimpleGradientBox(0, 0, 0, math.max(width - boostData.width, 0), baseBar:GetTall(), mainColor);
				CloudScript:DrawSimpleGradientBox(0, math.max(width - boostData.width, 0), 0, boostData.width, baseBar:GetTall(), hinderColor);
			end;
		else
			CloudScript:DrawSimpleGradientBox(0, 0, 0, width, baseBar:GetTall(), mainColor);
			CloudScript:DrawSimpleGradientBox(0, width, 0, math.min( boostData.width, baseBar:GetWide() ), baseBar:GetTall(), boostColor);
		end;
	end;
	...
	function self.progressBar.Paint(progressBar)
		...
		local uniqueID = self.attribute.uniqueID;
		local progress = CloudScript.attributes.panel.progress;
		local default = CloudScript.attributes.stored[uniqueID];
		...
		CloudScript:DrawSimpleGradientBox(0, 0, 0, progressBar:GetWide(), progressBar:GetTall(), color);
		CloudScript:DrawSimpleGradientBox(0, 0, 0, width, progressBar:GetTall(), progressColor);
	end;
...
vgui.Register("cloud_AttributesItem", PANEL, "DPanel");