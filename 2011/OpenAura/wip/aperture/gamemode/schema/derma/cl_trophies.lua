...
function PANEL:Rebuild()
	...
	local label = vgui.Create("aura_InfoText", self);
		label:SetText("Some trophies give you a reward when you achieve them.");
		label:SetInfoColor("blue");
	self.panelList:AddItem(label);
	
	for k, v in pairs( openAura.trophy:GetAll() ) do
		self.currentTrophy = v;
		self.panelList:AddItem( vgui.Create("aura_Trophy") );
	end;
...
vgui.Register("aura_Trophies", PANEL, "DFrame");
...
function PANEL:Init()
	...
	self.trophy = openAura.trophies:GetPanel().currentTrophy;
	self.nameLabel = vgui.Create("DLabel", self);
	self.nameLabel:SetText(self.trophy.name);
	...
	self.spawnIcon:SetToolTip(self.trophy.description);
	self.spawnIcon:SetSize(32, 32);
	
	if ( openAura.trophies:Has(self.trophy.name) ) then
		self.spawnIcon:SetImage("aperture/trophies/achieved");
	else
		self.spawnIcon:SetImage(self.trophy.image);
	end;
	...
	function self.progressBar.Paint(progressBar)
		...
		local progress = openAura.trophies:Get(self.trophy.name);
		local maximum = self.trophy.maximum;
...
-- Called each frame.
function PANEL:Think()
	self.progressLabel:SetText(openAura.trophies:Get(self.trophy.name).."/"..self.trophy.maximum);
	self.progressLabel:SizeToContents();
...
vgui.Register("aura_Trophy", PANEL, "DPanel");