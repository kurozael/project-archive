...
function PANEL:Init()
	...
	openAura.schema.lotteryPanel = self;
...
function PANEL:Rebuild()
	...
	local lotteryTicket = openAura.Client:GetSharedVar("lottery");
	local colorWhite = openAura.option:GetColor("white");
	local curTime = CurTime();
	
	if (nextLotteryTime > curTime) then
		self.timeLabel = vgui.Create("aura_InfoText", self);
		...
		if (!lotteryTicket) then
			...
			function lotterySubmit.DoClick(button)
				...
				openAura:StartDataStream( "PurchaseLottery", {numberOne, numberTwo, numberThree} );
			end;
		else
			local form = vgui.Create("DForm");
				form:SetName("Ticket");
...
vgui.Register("aura_Lottery", PANEL, "DFrame");