...
function PANEL:Init()
	...
	openAura.schema.billboardPanel = self;
...
function PANEL:Rebuild()
	...
	for k, v in ipairs(openAura.schema.billboards) do
		if ( v.data and IsValid(v.data.owner) ) then
			if (v.data.owner == openAura.Client) then
		...
		for k, v in ipairs(availableBillboards) do
			...
			if (v.data) then
				...
				function takeDownButton.DoClick(button)
					openAura:StartDataStream("TakeDownBillboard", v.id);
				end;
			else
			...
			function button.DoClick(button)
				openAura:StartDataStream( "UpdateBillboard", {
...
vgui.Register("aura_Billboards", PANEL, "DFrame");