...
if (SERVER) then
	...
	surface.CreateFont("cs", ScreenScale(60), 500, true, true, "AuraWeaponIcons");
	
	killicon.AddFont( SWEP.Classname, "CSKillIcons", SWEP.IconLetter, Color(255, 225, 0, 255) );
end

SWEP.HoldType = "ar2";
SWEP.Category = "OpenAura";
...
SWEP.Weight = 5;
SWEP.EjectDelay = 0;
SWEP.ShellEjectEffect = "aura_shelleject";
SWEP.ShellEjectAttachment = "2";

SWEP.Primary.ShotsToFire = 1;
...
SWEP.ShellEffect			= "aura_shelleject_rifle"
...
function SWEP:PrimaryAttack()
	...
	if self.Weapon:GetNetworkedBool("Silenced") == true then
		...
		self:CSShootBullet( self.SilencedDamage, self.SilencedRecoil, self.Primary.ShotsToFire, self.Primary.Spread )
	else
		self.Weapon:EmitSound(self.Primary.Sound, 200)
		self:CSShootBullet( self.Primary.Damage, self.Primary.Recoil, self.Primary.ShotsToFire, self.Primary.Spread )
	end
...
function SWEP:CSShootBullet( dmmg, recoil, amount, spread )
	if CLIENT then --in case if someone joined the server, and the font WASNT added.
		...
		if (self.Weapon:GetNetworkedBool("Silenced") or self.LowSmokeEffect) then
			...
			util.Effect("aura_smokeeffect", effectData);
