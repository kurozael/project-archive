...
function openAura.player:UpdateWeaponRaised(player)
	...
	if ( IsValid(weapon) ) then
		openAura:HandleWeaponFireDelay( player, bIsRaised, weapon, CurTime() );
	end;
end;

-- A function to get whether a player's weapon is raised.
function openAura.player:GetWeaponRaised(player, bIsCached)
	if (bIsCached) then
...
function openAura.player:SetWeaponRaised(player, bIsRaised)
	local weapon = player:GetActiveWeapon();
	...
	if ( IsValid(weapon) ) then
		if (type(bIsRaised) == "number") then
			...
			openAura:CreateTimer("auto_reapon_raised_"..player:UniqueID(), bIsRaised, 1, function()
