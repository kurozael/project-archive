...
openAura.config:AddModerator("weapon_selection_multi", "Should the weapon selection be hidden if the player has only one weapon.");
...
function PLUGIN:OverrideMarkupDraw(markupObject)
	function markupObject:Draw(xOffset, yOffset, hAlign, vAlign, alphaOverride)
		for k, v in pairs(self.blocks) do
			...
			openAura:OverrideMainFont(v.font);
				openAura:DrawSimpleText(v.text, x, y, Color(v.colour.r, v.colour.g, v.colour.b, alpha), hAlign, vAlign);
...
function PLUGIN:DrawWeaponInformation(itemTable, weapon, x, y, alpha)
	...
	local clipTwoAmount = openAura.Client:GetAmmoCount( weapon:GetSecondaryAmmoType() );
	local clipOneAmount = openAura.Client:GetAmmoCount( weapon:GetPrimaryAmmoType() );
	...
	or weapon.Instructions != "") ) ) then
		...
		if (itemTable and itemTable.description != "") then
			text = text..titleColor.."DESCRIPTIPON</color>\n"..textColor..openAura.config:Parse(itemTable.description).."</color>\n";
		end;
		...
		openAura.plugin:Call("PreDrawWeaponSelectionInfo", info);
		
		if (info.drawBackground) then
			openAura:DrawTexturedGradientBox( 2, x - 4, realY, 260, weaponMarkupHeight + 8, Color( backgroundColor.r, backgroundColor.g, backgroundColor.b, math.min(backgroundColor.a, alpha) ) );
		end;
