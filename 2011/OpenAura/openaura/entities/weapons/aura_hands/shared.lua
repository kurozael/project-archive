...
function SWEP:PunchEntity()
	...
	if (SERVER) then
		...
		if ( IsValid(traceLineAttack.Entity) ) then
			traceLineAttack.Entity:TakeDamageInfo( openAura:FakeDamageInfo(self.Primary.Damage, self, self.Owner, traceLineAttack.HitPos, DMG_CLUB, 1) );
		end;
	end;
end;
...
function SWEP:PrimaryAttack()
	if (SERVER) then
		if ( openAura.plugin:Call("PlayerCanThrowPunch", self.Owner) ) then
			...
			if (self.Owner:GetShootPos():Distance(trace.HitPos) <= 64) then
				if ( IsValid(trace.Entity) ) then
					if (trace.Entity:IsPlayer() or trace.Entity:IsNPC() or trace.Entity:GetClass() == "prop_ragdoll") then
						...
						and openAura.plugin:Call("PlayerCanPunchKnockout", self.Owner, trace.Entity) ) then
							openAura.player:SetRagdollState(trace.Entity, RAGDOLL_KNOCKEDOUT, 15);
							
							openAura.plugin:Call("PlayerPunchKnockout", self.Owner, trace.Entity);
						elseif ( openAura.plugin:Call("PlayerCanPunchEntity", self.Owner, trace.Entity) ) then
							self:PunchEntity();
							
							openAura.plugin:Call("PlayerPunchEntity", self.Owner, trace.Entity);
						end;
					...
					elseif ( IsValid( trace.Entity:GetPhysicsObject() ) ) then
						if ( openAura.plugin:Call("PlayerCanPunchEntity", self.Owner, trace.Entity) ) then
							self:PunchEntity();
			...
			openAura.plugin:Call("PlayerPunchThrown", self.Owner);
			...
			openAura.plugin:Call("PlayerAdjustNextPunchInfo", self.Owner, info);
...
function SWEP:SecondaryAttack()
	if (SERVER) then
		...
		if ( IsValid(trace.Entity) and openAura.entity:IsDoor(trace.Entity) ) then
			if (self.Owner:GetShootPos():Distance(trace.HitPos) <= 64) then
