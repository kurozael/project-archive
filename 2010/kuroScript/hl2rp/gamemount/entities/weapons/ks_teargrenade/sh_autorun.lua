...
if (CLIENT) then
	...
	SWEP.SlotPos = 5;
	SWEP.DrawAmmo = false;
	SWEP.PrintName = "Tear Grenade";
	SWEP.DrawCrosshair = true;
end

-- Set some information.
SWEP.Instructions = "Primary Fire: Throw.";
SWEP.Purpose = "Disorientating and knocking characters out with a heavy gas.";
SWEP.Contact = "";
...
function SWEP:CreateGrenade(power)
	if (SERVER) then
		...
		if ( ValidEntity(entity) ) then
			...
			-- Set some information.
			local trail = util.SpriteTrail(entity, entity:LookupAttachment("fuse"), Color(0, 0, 255), true, 8, 1, 1, (1 / 9) * 0.5, "sprites/bluelaser1.vmt");
			...
			timer.Simple(4, function()
				if ( ValidEntity(entity) ) then
					...
					-- Set some information.
					kuroScript.frame:CreateTimer("Tear Gas: "..tostring(effectData), 1, 20, function()
						for k, v in ipairs( g_Player.GetAll() ) do
							if ( v:HasInitialized() and !kuroScript.game:PlayerIsCombine(v) ) then
								if (v:GetPos():Distance(position) <= 512) then
									if ( kuroScript.player.CanSeePosition(v, position, 0.9, true) ) then
										if ( !v:IsRagdolled(RAGDOLL_FALLENOVER) ) then
											if (!v._NextTearGas or curTime >= v._NextTearGas) then
												v._NextTearGas = curTime + 30;
												
												-- Start a user message.
												umsg.Start("ks_TearGassed", v);
												umsg.End();
											end;
										end;
									end;
